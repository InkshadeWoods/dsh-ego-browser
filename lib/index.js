...
function resolveEgoEnv(cfg, { platform = process.platform, baseEnv = process.env } = {}) {
	...
	if (env.EGO_ISOLATE_SPACES === void 0) env.EGO_ISOLATE_SPACES = cfg?.isolateSpaces ? "1" : "0";
	return env;
}
...
function registerActionTools(ctx, cfg, reg) {
	...
	reg(t({
		...
		parameters: { name: {
			type: "string",
			required: true,
			description: "Task-space name. In persistent mode, ALWAYS specify 'default'. Reuse this single space for all browsing tasks."
		} },
